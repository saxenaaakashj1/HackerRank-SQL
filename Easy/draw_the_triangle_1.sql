...
WITH RECURSIVE
    pattern (n) AS (
        ...
        WHERE
            n > 1 -- Continue the recursion as long as 'n' is greater than 1
    )
SELECT
REPEAT ('* ', n) AS `Stars` -- For each value of 'n' generated by the CTE, repeat the '*' character 'n' times
FROM pattern;
-- Select from the 'pattern' CTE to produce the final output