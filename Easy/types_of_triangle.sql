...
WHEN A <> B
AND B <> C
AND C <> A THEN 'Scalene' END ELSE 'Not A Triangle' -- If the sides do not satisfy the triangle inequality theorem, it is not a valid triangle
END AS triangle_type -- Alias for the resulting column indicating the type of triangle
FROM triangles;
-- Specify the table containing the triangle side lengths